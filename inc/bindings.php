...
if ( ! function_exists( 'batavia_filter_query_loop_category' ) ) {
	/**
	 * Points a marked Query Loop block at the category chosen in Settings.
	 *
	 * Block bindings only cover a single attribute value, not a Query block's
	 * taxQuery, so Selected work and Notes carry a stable className instead --
	 * `batavia-query-portfolio` and `batavia-query-notes` -- and this
	 * reads the matching setting when the className is present. A block with
	 * neither className, or a setting left unset, is returned untouched, so
	 * it keeps whatever category its own Filters panel is already pointed at.
	 *
	 * Notes additionally carries a `mode` key: the category is only applied
	 * when batavia_get_category_scope() reports "specific", so choosing
	 * "All categories" in Settings actually means all, even if a category was
	 * chosen previously and left stored.
	 *
	 * @since 1.4.0
	 *
	 * @param array<string, mixed> $query_vars Query arguments for the block.
	 * @param WP_Block             $block      The Query block.
	 * @return array<string, mixed> Possibly-filtered query arguments.
	 */
	function batavia_filter_query_loop_category( $query_vars, $block ) {
		$markers = array(
			'batavia-query-portfolio' => array( 'category' => 'portfolio_category' ),
			'batavia-query-notes'     => array(
				'category' => 'notes_category',
				'mode'     => 'notes_category_mode',
			),
		);

		$class_name = isset( $block->parsed_block['attrs']['className'] )
			? (string) $block->parsed_block['attrs']['className']
			: '';

		foreach ( $markers as $marker => $config ) {
			if ( false === strpos( $class_name, $marker ) ) {
				continue;
			}

			if (
				isset( $config['mode'] )
				&& 'specific' !== batavia_get_category_scope( $config['mode'], $config['category'] )
			) {
				break;
			}

			$term_id = absint( batavia_get_setting( $config['category'] ) );

			if ( $term_id > 0 ) {
				$query_vars['tax_query'] = array( // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query -- One term from a theme setting, not user input at request time.
					array(
						'taxonomy' => 'category',
						'terms'    => array( $term_id ),
					),
...
add_filter( 'query_loop_block_query_vars', 'batavia_filter_query_loop_category', 10, 2 );
