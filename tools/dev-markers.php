<?php
/**
 * Plugin Name: Celestine dev markers
 ...
 */
function celestine_dev_mark_block( $block_content, $block ) {
...
add_filter( 'render_block', 'celestine_dev_mark_block', 10, 2 );
 ...
 * @return void
 */
function celestine_dev_mark_template() {
	...
	printf( "<!-- celestine: template = %s -->\n", esc_html( $template ) );
}
add_action( 'wp_head', 'celestine_dev_mark_template', 0 );
 ...
 * @return void
 */
function celestine_dev_mark_styles() {
	...
	foreach ( $queue as $handle ) {
		...
		if ( is_string( $src ) && false !== strpos( $src, '/themes/celestine/' ) ) {
			$mine[] = $handle;
		}
	}

	printf( "<!-- celestine: theme stylesheets in order = %s -->\n", esc_html( implode( ', ', $mine ) ) );
}
add_action( 'wp_footer', 'celestine_dev_mark_styles', 99 );
